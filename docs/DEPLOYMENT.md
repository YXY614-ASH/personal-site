...
用户已选择“先本地验收，保留 GitHub Pages 和 Vercel 部署配置”。V0.4 沿用该选择，不进行公网发布。已提供可运行的静态构建与两种托管配置，本地 HTTP、仓库子目录和页面资源已验证。
...
运行 `npm run dev`。服务仅监听 `127.0.0.1`，默认端口 4173，占用时自动递增至最多 4200。V0.4 支持文本 gzip、ETag 缓存校验和 HEAD；托管平台不运行此 Node 服务，传输压缩由平台负责。源码更改后运行 `npm run build` 刷新本地页面；服务逻辑变更需要重新启动服务。

## 版本归档与回溯

- 每次正式迭代提交功能、文档和测试记录，再创建对应版本 Tag。
- 本版本使用独立 `V0.4` 标签，执行 `git show V0.4` 可查看归档说明；此前标签保留历史版本。
